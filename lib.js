...
export function parseAmount(tx, locale, currency) {
  ...
  if (tx['amount-in']) return -parseCurrencyString(tx['amount-in'], locale, currency);
  if (tx['amount-out']) return parseCurrencyString(tx['amount-out'], locale, currency);
  return 0;
}

export function plainText(tx, locale, currency) {
  const accountPadLen = Math.max(43, tx.account1.length, tx.account2.length);
  const account1Amount = (-tx.amount).toLocaleString(locale, { style: 'currency', currency });
  const account2Amount = tx.amount.toLocaleString(locale, { style: 'currency', currency });
  const amountPadLen = Math.max(account1Amount.length, account2Amount.length);
  const balanceAmount = tx.balance.toLocaleString(locale, { style: 'currency', currency });
  ...
  output +=    `    ${tx.account1.padEnd(accountPadLen)} ${account1Amount.padStart(amountPadLen)} = ${balanceAmount}\n` +
